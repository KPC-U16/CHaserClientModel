...
def _initialPositionGrasp(values, target , initPosi) # 初期位置把握
...
def _initialAction(values, target, initPosi, i,wallCountA, wallCountB, itemCountA ,itemCountB, judgA, judgB, go) # 初期行動
...
def _obliqueItemGet(values,  random, go, tar) #斜めのアイテムを取りに行く
 if values[1] == 3
...
def _itemGet(values, go, tar) # 隣接するアイテムを取りに行く
  if values[2] == 3
    go = 0 # 上に歩く
    tar = 1 # walkする
  elsif values[4] == 3
...
def _obliqueEnemy(values, random, go, tar, look) #斜めに敵がいた時の行動
  if values[1] == 1
    rand = random.rand(0..1)
    if rand == 0
      go = 3 # 右に歩く
...
def _enemy(values, put, tar) # 隣接した敵に攻撃する
  if values[2] == 1
    put = 0 #上にputする
    tar = 0 # putする
  elsif values[4] == 1
...
def _avoidBlock(values, random, go, tar) # 壁にめり込まない
  if go == 0 # 上を向いているとき
    if values[2] == 2
      rand = random.rand(0..1)
      if rand == 0
...
def _act(put, go, look, tar, tarn, values, target) # 行動する
  case tar
    when 0
      case put
        when 0
...
_initialPositionGrasp(values, target , initPosi) # 初期位置把握

_initialAction(values, target, initPosi, i, wallCountA, wallCountB, itemCountA ,itemCountB, judgA, judgB, go) # 初期行動
...
_obliqueItemGet(values, random, go, tar) #斜めのアイテムを取りに行く

_obliqueEnemy(values, random, go, tar, look) #斜めに敵がいたときの行動

_avoidBlock(values, random, go, tar) # 壁にめり込まない

_itemGet(values, go, tar) # 隣接するアイテムを取りに行く

_enemy(values, put, tar) # 隣接する敵に攻撃を仕掛ける

_act(put, go, look, tar, tarn, values, target) #行動する
